        ...
        override this.TryResolve ( method : MethodInfo, target : obj, [<Out>] receiver : byref<GameCoordinatorReceiver> ) =
            match method.ReturnType with
                ...
                | t when t = typeof<unit> -> 
                    ...
                    success
                | _ -> false
    /// Provides a Func<IReceiveMethodResolver> that returns a new instance of the default F# receive method resolver
    let public FSharpReceiveMethodResolverProvider =
        Func<IReceiveMethodResolver>(fun () -> FSharpDefaultReceiveMethodResolver() :> IReceiveMethodResolver)
