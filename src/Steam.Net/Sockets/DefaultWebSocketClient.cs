...
{
    internal sealed class DefaultWebSocketClient : IWebSocketClient
    {
        ...
        private Func<byte[], Task> _func;
        private Func<Task> _connected;
        private Func<Exception, Task> _disconnected;
        private bool _isDisposed;

        public DefaultWebSocketClient(Func<byte[], Task> func, Func<Task> connected, Func<Exception, Task> disconnected)
        {
        ...
        private async Task RunAsync(CancellationToken cancelToken)
        {
            ...
            {
                await _disconnected(new TimeoutException("The operation timed out"));
            }
            catch (OperationCanceledException) { }
            catch (Exception ex)
            {
                await _disconnected(ex);
            }
        }
    }
}
